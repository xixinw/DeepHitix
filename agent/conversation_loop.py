...
def run_conversation(
    ...
    if isinstance(user_message, str):
        ...
        user_message = agent._apply_turn_routing(user_message)
    if isinstance(persist_user_message, str):
        persist_user_message = _sanitize_surrogates(persist_user_message)

    # Tell auxiliary_client what the live main provider/model are for
    # this turn after routing has chosen Flash vs Pro. Used by tools whose
    # behaviour depends on the active main model (e.g. vision_analyze's
    # native fast path) so they see the routed model instead of stale
    # config.yaml defaults. Idempotent — fine to call every turn.
    try:
        from agent.auxiliary_client import set_runtime_main
        set_runtime_main(
        ...
        while retry_count < max_retries:
            ...
            try:
                ...
                if hasattr(response, 'usage') and response.usage:
                    ...
                    agent._emit_llm_call_telemetry(
                        canonical_usage,
                        cost_result,
                        api_duration,
                        call_type="main",
    ...
    result = {
        ...
        "last_llm_call_event": getattr(agent, "_last_llm_call_event", None),
