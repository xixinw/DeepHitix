...
_MAIN_RUNTIME_FIELDS = (
    ...
    "deepseek_routing_enabled", "deepseek_flash_model", "deepseek_pro_model",
)
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if (
            _normalize_aux_provider(main_provider) == "deepseek"
            and runtime.get("deepseek_routing_enabled") is True
            and runtime.get("deepseek_flash_model")):
        # DeepSeek-native mode keeps user-visible chat on the routing policy
        # (default Flash, explicit /pro Pro) while side tasks should stay on
        # the cheap Flash lane unless the caller provided an explicit
        # auxiliary.<task>.model override.  This prevents compression,
        # title-generation, session-search, etc. from inheriting Pro after a
        # one-shot escalation.
        main_model = str(runtime["deepseek_flash_model"])
