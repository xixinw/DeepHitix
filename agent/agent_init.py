...
def init_agent(
    ...
    agent.model = model
    # DeepSeek-native Flash/Pro routing is off by default in upstream mode.
    # Downstream config/CLI layers can opt in by setting these attributes.
    agent.deepseek_routing_enabled = bool(deepseek_routing_enabled)
    agent.deepseek_flash_model = deepseek_flash_model or "deepseek-v4-flash"
    agent.deepseek_pro_model = deepseek_pro_model or "deepseek-v4-pro"
    agent._one_shot_pro_pending = False
    agent._last_routing_decision = "unknown"
    agent._last_escalation_reason = None
    ...
    if _selected_engine is not None:
        ...
        # Plugin context engines implement the public ContextEngine ABC; keep
        # the init-time update_model call limited to ABC-compatible kwargs.
        # DeepSeek-native routing kwargs are internal to the built-in
        # ContextCompressor path below.
        ...
        agent.context_compressor = ContextCompressor(
            ...
            deepseek_routing_enabled=agent.deepseek_routing_enabled,
            deepseek_flash_model=agent.deepseek_flash_model,
            deepseek_pro_model=agent.deepseek_pro_model,
        )
    ...
    agent._last_llm_call_event = None
