...
_OFFICIAL_DOCS_PRICING: Dict[tuple[str, str], PricingEntry] = {
    ...
    # DeepSeek
    # Source table is expressed per 1M tokens. ``deepseek-chat`` and
    # ``deepseek-reasoner`` are compatibility aliases for DeepSeek-V4-Flash
    # non-thinking / thinking modes respectively.
    (
        "deepseek",
        "deepseek-chat",
    ): PricingEntry(
        ...
        pricing_version="deepseek-pricing-2026-05-17",
    ),
    (
        "deepseek",
        "deepseek-reasoner",
    ...
    (
        ...
        source="official_docs_snapshot",
        source_url="https://api-docs.deepseek.com/quick_start/pricing",
        pricing_version="deepseek-pricing-2026-05-17-discounted",
    ),
    # Google Gemini
    (
        "google",
...
def _usage_field(obj: Any, name: str, default: Any = 0) -> Any:
    """Read a usage field from dicts, plain objects, or OpenAI SDK model_extra.

    Some OpenAI-compatible providers return provider-specific usage fields that
    the SDK preserves in ``model_extra`` rather than as declared attributes.
    """
    if obj is None:
        return default
...
def normalize_usage(
    ...
    *,
    ...
    if mode == "anthropic_messages" or provider_name == "anthropic":
        input_tokens = _to_int(_usage_field(response_usage, "input_tokens"))
        output_tokens = _to_int(_usage_field(response_usage, "output_tokens"))
        cache_read_tokens = _to_int(_usage_field(response_usage, "cache_read_input_tokens"))
        cache_write_tokens = _to_int(_usage_field(response_usage, "cache_creation_input_tokens"))
    elif mode == "codex_responses":
        input_total = _to_int(_usage_field(response_usage, "input_tokens"))
        output_tokens = _to_int(_usage_field(response_usage, "output_tokens"))
        details = _usage_field(response_usage, "input_tokens_details", None)
        cache_read_tokens = _to_int(_usage_field(details, "cached_tokens") if details else 0)
        cache_write_tokens = _to_int(
            _usage_field(details, "cache_creation_tokens") if details else 0
        )
        input_tokens = max(0, input_total - cache_read_tokens - cache_write_tokens)
    else:
        prompt_total = _to_int(_usage_field(response_usage, "prompt_tokens"))
        output_tokens = _to_int(_usage_field(response_usage, "completion_tokens"))
        details = _usage_field(response_usage, "prompt_tokens_details", None)

        if provider_name == "deepseek":
            # DeepSeek reports automatic context-cache buckets as top-level
            # prompt_cache_hit_tokens / prompt_cache_miss_tokens. These fields
            # are more specific than the generic OpenAI-compatible details and
            # are required for accurate cache-read vs input/miss costing.
            cache_read_tokens = _to_int(_usage_field(response_usage, "prompt_cache_hit_tokens"))
            cache_write_tokens = _to_int(
                _usage_field(details, "cache_write_tokens") if details else 0
            )
            cache_miss_tokens = _usage_field(response_usage, "prompt_cache_miss_tokens", None)
            if cache_miss_tokens is not None:
                input_tokens = _to_int(cache_miss_tokens)
            else:
                if not cache_read_tokens:
                    cache_read_tokens = _to_int(
                        _usage_field(details, "cached_tokens") if details else 0
                    )
            ...
            if not cache_write_tokens:
                cache_write_tokens = _to_int(
                    _usage_field(response_usage, "cache_creation_input_tokens")
                )
            input_tokens = max(0, prompt_total - cache_read_tokens - cache_write_tokens)
