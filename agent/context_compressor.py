...
class ContextCompressor(ContextEngine):
    ...
    def update_model(
        ...
        provider: str = "",
        api_mode: str = "",
        deepseek_routing_enabled: bool | None = None,
        ...
        if deepseek_routing_enabled is not None:
            self.deepseek_routing_enabled = bool(deepseek_routing_enabled)
        if deepseek_flash_model:
            self.deepseek_flash_model = deepseek_flash_model
        if deepseek_pro_model:
            self.deepseek_pro_model = deepseek_pro_model
    ...
    def __init__(
        ...
        deepseek_routing_enabled: bool = False,
        ...
        self.api_mode = api_mode
        self.deepseek_routing_enabled = bool(deepseek_routing_enabled)
        self.deepseek_flash_model = deepseek_flash_model or "deepseek-v4-flash"
        self.deepseek_pro_model = deepseek_pro_model or "deepseek-v4-pro"
        ...
        # When summary generation fails and a fallback checkpoint is inserted,
        # record how many turns were compressed so callers (gateway hygiene,
        # /compress) can surface a visible warning.
...
The user has requested that this compaction PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail — exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget. Even for the focus topic, NEVER preserve API keys, tokens, passwords, or credentials — use [REDACTED]."""
        ...
        try:
            ...
            if self.deepseek_routing_enabled:
                main_runtime.update({
                    "deepseek_routing_enabled": True,
                    "deepseek_flash_model": self.deepseek_flash_model,
    ...
    def _generate_extractive_checkpoint(
        self,
        ...
        *,
        ...
        """Build a non-LLM checkpoint when summary generation fails.

        This is deliberately extractive and conservative: preserve concrete
        snippets from the compressed turns (recent user asks, assistant state,
        tool calls/results, paths/commands embedded in those snippets) rather
        than inventing a narrative. It is a continuity safety net for provider
        outages or bad auxiliary model config, not a replacement for the normal
        structured LLM summary.
        """
        serialized = self._serialize_for_summary(turns_to_summarize)
        max_chars = min(16_000, max(4_000, self.max_summary_tokens * _CHARS_PER_TOKEN))
        if len(serialized) > max_chars:
            head = int(max_chars * 0.65)
            marker = "\n...[extractive checkpoint truncated]...\n"
            tail = max_chars - head - len(marker)
            serialized = serialized[:head] + marker + serialized[-max(tail, 0):]

        latest_user = "None."
        for msg in reversed(turns_to_summarize):
            if msg.get("role") == "user":
                latest_user = redact_sensitive_text(_content_text_for_contains(msg.get("content"))).strip() or "None."
                if len(latest_user) > 1_000:
                    latest_user = latest_user[:997].rstrip() + "..."
                break
        ...
        body = (
            ...
            f"{latest_user}\n\n"
            "## Critical Context\n"
            "Summary generation failed; Extractive checkpoint fallback used. "
            f"{dropped_count} message(s) were compressed with a deterministic extractive checkpoint. "
            "Facts below are copied from prior turns and may be partial, but they are not empty placeholders.\n"
        )
        if focus_topic:
            body += f"Focus topic: {redact_sensitive_text(focus_topic)}\n"
        body += (
            "\n## Extracted Prior Turns\n"
            f"{serialized}\n\n"
            "## Remaining Work\n"
            "Use the protected live tail messages after this summary as the active source of truth, "
            "and use this extractive checkpoint only to recover details that would otherwise be lost."
        )
        body = redact_sensitive_text(body)
        self._previous_summary = self._strip_summary_prefix(body)
    ...
    def compress(self, messages: List[Dict[str, Any]], current_tokens: int = None, focus_topic: str = None, force: bool = False) -> List[Dict[str, Any]]:
        ...
        #   False → Fall through to the fallback path below: insert a
        #           deterministic extractive checkpoint and drop the middle
        #           window. Records _last_summary_fallback_used /
        ...
        # False (the default). Insert a deterministic extractive checkpoint so
        # the model can recover concrete task state rather than receiving an
        # empty "summary unavailable" marker.
        if not summary:
            if not self.quiet_mode:
                logger.warning("Summary generation failed — inserting extractive checkpoint fallback")
            n_dropped = compress_end - compress_start
            self._last_summary_dropped_count = n_dropped
            self._last_summary_fallback_used = True
            summary = self._generate_extractive_checkpoint(
                turns_to_summarize,
