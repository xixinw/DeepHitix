...
def test_format_footer_cost_field_uses_safe_llm_event_summary():
    out = format_runtime_footer(
    ...
    assert out.startswith("deepseek-chat · 💰 Turn cost: ~$0.0002")
    assert "tokens 1,050" in out
    assert "cache 400/1,000" in out
    assert "do not leak" not in out


def test_format_footer_cost_field_drops_when_event_missing():
    out = format_runtime_footer(
        model="openai/gpt-5.4",
        context_tokens=50,
        context_length=100,
        cwd="/tmp/wd",
        fields=("cost", "context_pct"),
        llm_call_event=None,
    )
    assert out == "50%"
...
def test_build_footer_can_include_optional_cost_field():
    out = build_footer_line(
        user_config={
            "display": {
                "runtime_footer": {"enabled": True, "fields": ["model", "cost"]}
            }
        },
        platform_key="telegram",
        model="deepseek/deepseek-chat",
