...
class TestNonStringContent:
    ...
    def test_summary_call_passes_deepseek_routing_runtime(self):
        """Compression must expose routing flags so auxiliary auto can pick Flash."""
        mock_response = MagicMock()
        mock_response.choices = [MagicMock()]
        mock_response.choices[0].message.content = "ok"
...
class TestSummaryFailureTrackingForGatewayWarning:
    ...
    summary-generation failure inserts an extractive checkpoint fallback and
    ...
    def test_compress_records_fallback_and_dropped_count_on_summary_failure(self):
        ...
        # Result must still be well-formed (fallback summary present), but the
        # fallback must be an extractive checkpoint rather than an empty marker.
        assert any(
            isinstance(m.get("content"), str) and "Extractive checkpoint fallback used" in m["content"]
            for m in result
        )

    def test_summary_failure_uses_extractive_checkpoint_not_empty_placeholder(self):
        with patch("agent.context_compressor.get_model_context_length", return_value=100000):
            c = ContextCompressor(model="test", quiet_mode=True, protect_first_n=2, protect_last_n=2)

        msgs = [
            {"role": "system", "content": "sys"},
            {"role": "user", "content": "start the continuity fix"},
            {"role": "assistant", "content": "plan: preserve task state"},
            {"role": "user", "content": "Active task: implement extractive fallback in agent/context_compressor.py"},
            {
                "role": "assistant",
                "content": "I will patch tests/agent/test_context_compressor.py and then run pytest.",
                "tool_calls": [
                    {
                        "id": "call_terminal",
                        "type": "function",
                        "function": {
                            "name": "terminal",
                            "arguments": '{"command": "pytest tests/agent/test_context_compressor.py -q"}',
                        },
                    }
                ],
            },
            {
                ...
                "content": '{"output": "1 failed, 120 passed", "exit_code": 1}',
            },
            {"role": "assistant", "content": "RED confirmed for the fallback test"},
            {"role": "user", "content": "continue into green"},
            {"role": "assistant", "content": "preparing minimal implementation"},
            {"role": "user", "content": "latest live user message stays in protected tail"},
        ]

        with patch("agent.context_compressor.call_llm", side_effect=Exception("summary provider down")):
            result = c.compress(msgs)

        fallback_text = "\n".join(
            m.get("content", "") for m in result if isinstance(m.get("content"), str)
        )
        assert "Extractive checkpoint fallback used" in fallback_text
        assert "Summary generation was unavailable" not in fallback_text
        assert "Active task: implement extractive fallback" in fallback_text
        assert "agent/context_compressor.py" in fallback_text
        assert "tests/agent/test_context_compressor.py" in fallback_text
        assert "pytest tests/agent/test_context_compressor.py -q" in fallback_text
        assert "1 failed, 120 passed" in fallback_text
