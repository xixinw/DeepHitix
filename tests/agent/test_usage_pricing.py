...
def test_normalize_usage_deepseek_uses_prompt_cache_hit_and_miss_tokens():
    usage = SimpleNamespace(
        prompt_tokens=1000,
...
def test_normalize_usage_deepseek_handles_missing_cache_fields_as_miss_tokens():
    usage = SimpleNamespace(
        prompt_tokens=1000,
        completion_tokens=200,
        total_tokens=1200,
...
def test_normalize_usage_deepseek_reads_completion_reasoning_tokens():
    usage = SimpleNamespace(
        prompt_tokens=1000,
...
def test_deepseek_v4_flash_alias_pricing_includes_cache_hit_rate():
    """DeepSeek compatibility aliases map to V4-Flash pricing.

    Cached input must be priced instead of marking the whole turn unknown.
    """
...
def test_deepseek_chat_estimate_usage_cost_with_cache_hits():
    """Ensure cached deepseek-chat turns get a dollar estimate, not unknown."""
    result = estimate_usage_cost(
        "deepseek-chat",
        CanonicalUsage(input_tokens=1000000, cache_read_tokens=500000, output_tokens=500000),
        provider="deepseek",
    )

    assert result.status == "estimated"
    assert result.amount_usd is not None
    # 1M cache-miss input × $0.14/M + 500K cache-hit input × $0.0028/M +
    # 500K output × $0.28/M = $0.14 + $0.0014 + $0.14 = $0.2814
    assert float(result.amount_usd) == 0.2814
