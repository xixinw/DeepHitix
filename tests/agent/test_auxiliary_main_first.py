...
class TestResolveAutoMainFirst:
    ...
    def test_deepseek_native_routing_uses_flash_for_text_auxiliary(self):
        """DeepSeek-native routing enabled → text auxiliary defaults to Flash, not Pro."""
        with patch(
            "agent.auxiliary_client._read_main_provider",
            ...
            mock_client = MagicMock()
            mock_resolve.return_value = (mock_client, "deepseek-v4-flash")
        ...
        assert mock_resolve.call_args.args[0] == "deepseek"
        assert mock_resolve.call_args.args[1] == "deepseek-v4-flash"
