...
class AIAgent:
    ...
    def __init__(
        ...
        init_agent(
            ...
            deepseek_routing_enabled=deepseek_routing_enabled,
    ...
    def _apply_turn_routing(self, user_message: str) -> str:
        """Apply the per-turn Flash/Pro routing policy and return API message text.

        Routing metadata is stored as enum-like fields for telemetry. User text
        is only returned as the sanitized message; it is never stored in the
        routing reason fields.
        """
        decision = resolve_turn_routing(
            user_message,
            RoutingPolicyConfig(
                enabled=bool(getattr(self, "deepseek_routing_enabled", False)),
                flash_model=getattr(self, "deepseek_flash_model", "") or "",
                pro_model=getattr(self, "deepseek_pro_model", "") or "",
                current_model=getattr(self, "model", "") or "",
            ),
            one_shot_pro_pending=bool(getattr(self, "_one_shot_pro_pending", False)),
        )
        self.model = decision.model
        self._last_routing_decision = decision.routing_decision
        self._last_escalation_reason = decision.escalation_reason
        if decision.consume_one_shot_pro:
            self._one_shot_pro_pending = False
        return decision.sanitized_message
    ...
    def _emit_llm_call_telemetry(
        ...
        *,
        ...
        """Best-effort append-only telemetry for one successful LLM call.

        Only canonical counters and metadata are emitted.  Request messages,
        assistant text, tool results, headers, and secrets are intentionally not
        passed into the event builder.
        """
        try:
            amount = getattr(cost_result, "amount_usd", None)
            tool_schema_hash = compute_tool_schema_hash(getattr(self, "tools", None))
            immutable_prefix_hash = compute_immutable_prefix_hash(getattr(self, "_last_api_messages", None))
            previous_event = getattr(self, "_last_llm_call_event", None)
            prefix_drift_reason = None
            if isinstance(previous_event, dict):
                if previous_event.get("tool_schema_hash") != tool_schema_hash:
                    prefix_drift_reason = "tool_schema_changed"
                if previous_event.get("immutable_prefix_hash") != immutable_prefix_hash:
                    prefix_drift_reason = (
                        "tool_schema_and_immutable_prefix_changed"
                        if prefix_drift_reason
                        else "immutable_prefix_changed"
                    )
            event = build_llm_call_event(
                ...
                turn_id=str(getattr(self, "_api_call_count", "")) or None,
                estimated_cost_usd=float(amount) if amount is not None else None,
                pricing_source=getattr(cost_result, "source", "none"),
                pricing_status=getattr(cost_result, "status", "unknown"),
                latency_ms=int(max(api_duration, 0) * 1000),
                routing_decision=getattr(self, "_last_routing_decision", "unknown"),
                escalation_reason=getattr(self, "_last_escalation_reason", None),
            ...
            if getattr(self, "platform", None) == "cli":
                self._vprint(f"{self.log_prefix}   {format_llm_call_summary(event)}")
            self._last_llm_call_event = event
            return append_telemetry_event(event)
        except Exception:
            logger.debug("LLM call telemetry emission failed", exc_info=True)
            return False
    ...
    def reset_session_state(self):
        ...
        self._last_llm_call_event = None
